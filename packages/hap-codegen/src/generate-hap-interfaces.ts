...
const interfacesPath = path.resolve('../hap-fluent/src/types/hap-interfaces.ts');
...
export function extractEnumsFromCharacteristic(char: Characteristic): string[] {
  ...
  const staticProps = Object.getOwnPropertyNames(ctor).filter(
    (key) => key === key.toUpperCase() && typeof (ctor as any)[key] !== 'function' && key !== 'UUID'
  );
...
function extractCharacteristicsFromService(service: Service): ServiceCharacteristics {
  const required: {
    UUID: string;
    name: string;
    className: string;
  ...
  if (required.length === 0 && optional.length === 0) {
    console.warn(`Warning: Service ${service.displayName} has no characteristics defined.`);
  }
  ...
  return Object.entries(Service)
    .map(([name, service]) => {
      if (typeof service !== 'function' && !(service.prototype instanceof Service)) {
        console.warn(`Skipping ${name} as it is not a valid Service constructor.`);
        return null;
      }
      console.warn(`Extracting service ${name} of type ${service.name}.`);
      const serviceInstance = new service() as Service;

      const { required, optional } = extractCharacteristicsFromService(serviceInstance);
      return {
        name,
        required,
        optional,
...
function extractCharacteristicsFromNamespace() {
  return Object.entries(Characteristic).reduce(
    (p, [name, char]) => {
      if (typeof char !== 'function' || !(char.prototype instanceof Characteristic)) {
        console.warn(`Skipping ${name} as it is not a valid Characteristic instance.`);
        return p;
      } else {
        console.warn(
