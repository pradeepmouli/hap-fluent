...
describe("Integration Tests", () => {
  ...
  beforeEach(() => {
    mockAccessory = createPlatformAccessory(
      "Test Device",
      "test-uuid-12345678-1234-1234-1234-123456789123",
    );
    mockPlugin = {
      log: {
        info: vi.fn(),
  ...
  describe("Thermostat Control", () => {
    it("should control thermostat through full stack", async () => {
      ...
      const services = createServicesObject(thermostatService as any) as any;
    ...
    it("should handle thermostat state changes with handlers", async () => {
      ...
      services.thermostat.characteristics.targetTemperature.onGet(getHandler).onSet(setHandler);
  ...
  describe("Property Chaining and Updates", () => {
    it("should support complex chaining across services", () => {
      ...
      // Complex chaining
      services.lightbulb.characteristics.on.set(true).setProps({ minValue: 0, maxValue: 1 });
  ...
  describe("Context Management", () => {
    it("should maintain context across accessory lifecycle", async () => {
      ...
      expect((handler.context as any).deviceId).toBe("ABC123");
      expect((handler.context as any).firmwareVersion).toBe("1.2.3");
      expect((handler.context as any).customData).toEqual({ key: "value" });
    ...
    it("should allow context modifications", () => {
      ...
      (handler.context as any).count++;
      expect((handler.context as any).count).toBe(1);

      (handler.context as any).count = 5;
      expect((mockAccessory.context as any).count).toBe(5);
    });
  });
  ...
  describe("Real-world Scenarios", () => {
    ...
    it("should simulate climate control automation", async () => {
      ...
      services.thermostat.characteristics.currentTemperature.onGet(async () => currentTemp);
    ...
    it("should handle multi-outlet power strip", () => {
      ...
      const outlets = Array.from({ length: 4 }, (_, i) => {
        const outlet = new MockService(`Outlet ${i + 1}`, `outlet${i + 1}`);
