    ...
    (acc: ServicesObject<T>, service): ServicesObject<T> => {
      ...
      if (service?.subtype) {
        ...
        if (serviceObject && hasSubTypes(serviceObject as ServiceMap[keyof ServiceMap])) {
          ...
          serviceObject = {
            ...serviceObject,
            [camelcase(service.subtype, { pascalCase: true })]: wrapService(service),
          };
        } else {
        ...
        if (serviceClass) {
          ...
          for (const charKey in stateValue) {
            const characteristic = service.getCharacteristic(
              (api.hap.Characteristic as Record<string, any>)[
                camelcase(charKey, { pascalCase: true })
              ],
            );
            if (characteristic) {
...
export class AccessoryHandler<TContext extends UnknownContext, Services extends Interfaces[]> {
  ...
  public async initialize(
    apiOrState?: API | InternalServicesStateObject<Services>,
    maybeState?: InternalServicesStateObject<Services>,
  ): Promise<void> {
    ...
    if (api && initialState) {
      ...
      for (const key in initialState) {
        ...
        if (typeof stateValue === "object" && !isMultiService(stateValue as any)) {
          ...
          const service = this.accessory.services.find(
            (s) =>
    ...
    this.services = createServicesObject(
      ...(this.accessory.services as unknown as InstanceType<
        ServiceForInterface<Services[number]>
      >[]),
    ) as ServicesObject<Services>;
  }
   ...
   * ```
   */
  public addService(serviceClass: any, displayName: string, subtype?: string): FluentService<any> {
    const service = this.accessory.addService(
      new serviceClass(displayName, subtype) as unknown as any,
    );
