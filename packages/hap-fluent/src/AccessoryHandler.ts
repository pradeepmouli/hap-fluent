...
export function createServicesObject<T extends Interfaces[]>(
  ...
  return services.reduce<ServicesObject<T>>(
    (acc: ServicesObject<T>, service): ServicesObject<T> => {
      const serviceName = camelcase(service.constructor.name /* get service name */, {
        pascalCase: false,
      }) as keyof ServicesObject<T>;
      ...
      if (service?.subtype) {
        ...
        if (hasSubTypes(serviceObject)) {
          ...
          serviceObject = {
            ...serviceObject,
            [camelcase(service.subtype, { pascalCase: true })]: wrapService(service as any),
          };
        } else {
          //@ts-ignore
  ...
  for (const key in initialState) {
    if (typeof initialState[key] === "object") {
      const serviceClass = Service[
        camelcase(key, { pascalCase: true }) as keyof InterfaceMap
      ] as WithUUID<typeof Service.AccessoryInformation>;
      if (serviceClass) {
        ...
        if (isMultiService(initialState[key] as any)) {
          ...
          const wrappedService = wrapService(
            service as InstanceType<typeof serviceClass>,
          ) as FluentService<any>;
          //@ts-expect-error
          services[camelcase(key, { pascalCase: true }) as keyof InternalServicesObject<Services>] =
            wrappedService as any;
          for (const charKey in initialState[key]) {
            if (charKey in wrappedService.characteristics) {
              const characteristic = wrappedService.characteristics[
                charKey as keyof typeof wrappedService.characteristics
              ] as Characteristic;
              if (characteristic) {
        ...
        if (typeof initialState[key] === "object") {
          ...
          if (serviceClass) {
            ...
            if (isMultiService(initialState[key] as any)) {
              ...
              const service =
                this.accessory.getService(serviceClass as WithUUID<typeof Service>) ||
                (this.accessory.addService(new serviceClass()) as InstanceType<
                  typeof serviceClass
                >);
              ...
              this.services[
                camelcase(key, { pascalCase: true }) as keyof ServicesObject<Services>
              ] = wrappedService as any;
              for (const charKey in initialState[key]) {
                if (charKey in wrappedService.characteristics) {
                  const characteristic = wrappedService.characteristics[
