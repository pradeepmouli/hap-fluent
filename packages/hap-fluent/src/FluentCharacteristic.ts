...
export class FluentCharacteristic<T extends CharacteristicValue> {
   ...
   */
  log(): this {
    this.interceptors.push(createLoggingInterceptor());
    return this;
  }
  ...
  limit(maxCalls: number, windowMs: number): this {
    this.interceptors.push(createRateLimitInterceptor(maxCalls, windowMs));
    return this;
  }
  ...
  clamp(min: number, max: number): this {
    this.interceptors.push(createClampingInterceptor(min, max));
    return this;
  }
  ...
  transform(transformFn: (value: CharacteristicValue) => CharacteristicValue): this {
    this.interceptors.push(createTransformInterceptor(transformFn));
    return this;
  }
  ...
  audit(maxEntries = 1000): this {
    ...
    this.interceptors.push({
      beforeSet(value, context) {
        if (auditTrail.length >= maxEntries) {
          auditTrail.shift();
        }
        auditTrail.push({
          operation: 'set',
  ...
  codec(
    ...
    this.interceptors.push(createCodecInterceptor(encode, decode));
    return this;
  }
