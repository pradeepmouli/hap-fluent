...
import { Enums, wrapAccessory } from "../src/index.js";
...
function example1TemperatureConversion(
  ...
  const tempChar = handler.services.temperatureSensor.characteristics.currentTemperature
    .codec(
...
function example2StringFormatting(
  ...
  const handler = wrapAccessory(plugin, platformAccessory, api).with(Service.Lightbulb, "Light");

  // Convert strings to/from uppercase
  const nameChar = handler.services.lightbulb.characteristics.name.codec(
...
function example3JsonCodec(
  ...
  const configChar = handler.services.lightbulb.characteristics.name// Using Name as example storage
    .codec(
...
function example4PercentageConversion(
  ...
  const brightnessChar = handler.services.lightbulb.characteristics.brightness
    .codec(
...
function example6ValueMapping(
  ...
  const modeChar = handler.services.thermostat.characteristics.targetHeatingCoolingState
    .codec(
  ...
  modeChar.set(Enums.TargetHeatingCoolingState.Heat);
