...
import { wrapAccessory, type Outlet, Thermostat, HumiditySensor, OccupancySensor } from '../src/index.js';
...
export function addingServicesExample(
	...
	const handler2 = handler.with(Service.MotionSensor, 'Motion Detector');
...
export function multiServiceAccessoryExample(
	...
	handler2.services.fanv2.characteristics.Active.onSet(async (value: number) => {
...
export function handlerBasedExample(
	...
	const handler = wrapAccessory(plugin, platformAccessory, api)
		.with(Service.Lightbulb, 'Main Light');
...
export function smartLightExample(
	...
	const handler = wrapAccessory(plugin, platformAccessory, api)
		.with(Service.Lightbulb, 'Smart Light');
