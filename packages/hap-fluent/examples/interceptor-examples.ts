...
console.log("=== Example 1: Logging on onSet ===");
{
  ...
  const service = accessory.addService(Service.Lightbulb, "My Light") as InstanceType<
    typeof Service.Lightbulb
  >;
  service.addCharacteristic(Characteristic.Brightness);

  const wrapped = wrapService<typeof Service.Lightbulb>(service);
...
console.log("\n=== Example 2: Rate Limiting on onSet ===");
{
  ...
  wrapped.characteristics.brightness.limit(3, 1000).onSet(async (value) => {
...
console.log("\n=== Example 3: Value Clamping on onSet ===");
{
  ...
  wrapped.characteristics.brightness.clamp(0, 100).onSet(async (value) => {
...
console.log("\n=== Example 5: Audit Trail ===");
{
  ...
  const wrapped = wrapService<typeof Service.LockMechanism>(service);

  // Track all lock state changes with audit trail
  wrapped.characteristics.lockTargetState.audit().onSet(async (value) => {
