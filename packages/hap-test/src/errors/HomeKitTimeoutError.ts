...
export class HomeKitTimeoutError extends Error {
	...
	constructor(
		...
		const message = [
			...
			...(suggestions.length > 0 ? [``, `💡 Troubleshooting:`, ...suggestions.map(s => `  • ${s}`)] : []),
		].join('\n');
	...
	private static generateSuggestions(operation: string, timeoutMs: number, context?: any): string[] {
		...
		if (operation.includes('response') || operation.includes('read') || operation.includes('write')) {
		...
		if (timeoutMs < 1000) {
			suggestions.push(`Timeout is quite short (${timeoutMs}ms) - consider increasing if operations are legitimately slow`);
		}
