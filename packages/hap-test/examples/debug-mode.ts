...
async function demonstrateDebugMode() {
  ...
  service.addCharacteristic(new MockCharacteristic('Brightness', 'Brightness', 0, {
    format: 'int',
    perms: ['pr', 'pw', 'ev'],
    minValue: 0,
    maxValue: 100,
  ...
  const brightness = await harness1.homeKit.characteristic('light-uuid', 'Lightbulb', 'Brightness')?.getValue();
