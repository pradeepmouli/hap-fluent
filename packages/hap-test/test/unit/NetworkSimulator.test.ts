...
describe('NetworkSimulator', () => {
	...
	describe('Packet Loss', () => {
		it('should throw error when packet is lost', async () => {
			...
			await expect(
				simulator.applyConditions(async () => 'result')
		...
		it('should reject invalid packet loss rates', () => {
			expect(() => simulator.setPacketLoss(-0.1)).toThrow('Packet loss rate must be between 0 and 1');
			expect(() => simulator.setPacketLoss(1.1)).toThrow('Packet loss rate must be between 0 and 1');
		});
