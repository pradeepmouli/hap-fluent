...
import { MockAccessory, MockService, MockCharacteristic, MockHomeKit } from '../../src/MockHomeKit.js';
...
describe('Multi-User and Batch Operations', () => {
  ...
  describe('Batch Operations', () => {
    beforeEach(() => {
      ...
      lightService.addCharacteristic(new MockCharacteristic('On', 'On', true, { format: 'bool', perms: ['pr', 'pw'] }));
      lightService.addCharacteristic(new MockCharacteristic('Brightness', 'Brightness', 75, { format: 'int', perms: ['pr', 'pw'], minValue: 0, maxValue: 100 }));
      ...
      switchService.addCharacteristic(new MockCharacteristic('On', 'On', false, { format: 'bool', perms: ['pr', 'pw'] }));
    ...
    it('refreshAll() skips write-only characteristics', async () => {
      ...
      svc.addCharacteristic(new MockCharacteristic('WriteOnly', 'WriteOnly', 0, { format: 'int', perms: ['pw'] }));
      acc.addService(svc);
