...
function buildLight(uuid: string, name: string): MockAccessory {
  ...
  service.addCharacteristic(
    new MockCharacteristic("Brightness", "Brightness", 100, {
      format: "int",
      perms: ["pr", "pw", "ev"],
      minValue: 0,
      maxValue: 100,
...
describe("Multi-accessory platform", () => {
  ...
  beforeEach(async () => {
    ...
    accessories = [buildLight("light-0", "Hallway Light"), buildLight("light-1", "Kitchen Light")];
  ...
  it("restores cached accessories and supports dynamic discovery", async () => {
    ...
    contactService.addCharacteristic(
      new MockCharacteristic("ContactSensorState", "ContactSensorState", 0, {
        format: "int",
        perms: ["pr", "pw", "ev"],
        validValues: [0, 1],
      }),
