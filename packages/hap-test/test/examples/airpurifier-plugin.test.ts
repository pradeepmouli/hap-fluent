...
import { toHaveService, toHaveAccessory, toHaveCharacteristic, toHaveValue, toBeInRange } from '../../src/matchers/index.js';
...
describe('AirPurifier platform', () => {
  ...
  beforeEach(async () => {
    ...
    svc.addCharacteristic(new MockCharacteristic('Active', 'Active', Active.Inactive, {
      format: 'int', perms: ['pr', 'pw', 'ev'], validValues: [Active.Inactive, Active.Active],
    }));
    svc.addCharacteristic(new MockCharacteristic('CurrentAirPurifierState', 'CurrentAirPurifierState', CurrentState.Inactive, {
      format: 'int', perms: ['pr', 'pw', 'ev'], validValues: [CurrentState.Inactive, CurrentState.Idle, CurrentState.Purifying],
    }));
    svc.addCharacteristic(new MockCharacteristic('TargetAirPurifierState', 'TargetAirPurifierState', TargetState.Auto, {
      format: 'int', perms: ['pr', 'pw', 'ev'], validValues: [TargetState.Manual, TargetState.Auto],
    }));

    // Speed and filter
    svc.addCharacteristic(new MockCharacteristic('RotationSpeed', 'RotationSpeed', 0, {
      format: 'int', perms: ['pr', 'pw', 'ev'], minValue: 0, maxValue: 100, unit: 'percentage',
    }));
    svc.addCharacteristic(new MockCharacteristic('FilterLifeLevel', 'FilterLifeLevel', 100, {
      format: 'int', perms: ['pr', 'pw', 'ev'], minValue: 0, maxValue: 100, unit: 'percentage',
    }));
    svc.addCharacteristic(new MockCharacteristic('FilterChangeIndication', 'FilterChangeIndication', FilterChange.OK, {
      format: 'int', perms: ['pr', 'pw', 'ev'], validValues: [FilterChange.OK, FilterChange.Change],
    }));
  ...
  it('activates purifier and transitions states', async () => {
    ...
    const current = harness.homeKit.characteristic('purifier-uuid', 'AirPurifier', 'CurrentAirPurifierState')!;
    const target = harness.homeKit.characteristic('purifier-uuid', 'AirPurifier', 'TargetAirPurifierState')!;
  ...
  it('updates filter change indication when life is low', async () => {
    ...
    const indication = harness.homeKit.characteristic('purifier-uuid', 'AirPurifier', 'FilterChangeIndication')!;
