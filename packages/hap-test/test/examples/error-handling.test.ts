...
describe("Error handling with simulated networks", () => {
  ...
  beforeEach(async () => {
    ...
    targetChar = harness.homeKit.characteristic(
      "thermostat-uuid",
      "Thermostat",
      "TargetTemperature",
    )!;
  ...
  it("throws when packet loss is triggered", async () => {
    ...
    await expect(targetChar.getValue()).rejects.toMatchObject({
      errorType: NetworkErrorType.PACKET_LOSS,
    });
  });

  it("errors on disconnect and recovers after reconnect", async () => {
    simulator.disconnect();
    await expect(targetChar.setValue(21)).rejects.toMatchObject({
      errorType: NetworkErrorType.DISCONNECTED,
    });
