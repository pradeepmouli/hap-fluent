# Phase 0: Testing Gaps — refactor-002
...
- packages/hap-fluent/src/AccessoryHandler.ts
  ...
  - `getOrAddService()` (legacy wrapper to be delegated)
  - `AccessoryHandler.initialize()` (new/aliased instance method)
...
1. Handler Creation
   - Given an accessory, `wrapAccessory(accessory)` returns an `AccessoryHandler` bound to that accessory.
   - Verify returned handler methods operate on the provided accessory.

2. Initialization Behavior
   - `AccessoryHandler.initialize()` produces identical side effects to `initializeAccessory(accessory)`:
     - Registration/wiring with Homebridge API
     - Interceptors and logging attached
     - Any error handling paths preserved

3. Service Addition Idempotency
   - `AccessoryHandler.addService(type, subtype?)` returns existing service if present; otherwise creates, registers, and returns a new one.
   - No duplicate services are created across repeated calls.

4. Error Handling
   - Invalid service type/subtype produce the same errors as before (types in packages/hap-fluent/src/errors.ts).

5. Integration Paths
   - End-to-end flows in property-based tests and integration tests remain unchanged:
...
- [ ] Run full suite: build, lint, type-check, unit, property-based, integration.
